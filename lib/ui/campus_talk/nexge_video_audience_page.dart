...
class TalkAudiencePageNexge extends StatefulWidget {
  final int? eventId;
  ...
  @override
  TalkAudiencePageNexgeState createState() => TalkAudiencePageNexgeState(
      eventId: eventId,
      mute: mute,
...
class TalkAudiencePageNexgeState extends State<TalkAudiencePageNexge> {
  ...
  Future getAccessToken() async {
    try {
      ...
      http.post(Uri.parse('https://conf-signalling.nexge.com/api/v1/auth/login'), body: {"conferenceId":eventId.toString()}).then(
              (response) {
  ...
  Future<void> connectorMuteCall(bool isMute) async {
    ...
    _participantNotifier!
        .update(db, prefs!.getInt(Strings.userId), isMute ? 0 : 1);
  ...
  Future<void> turnOnOffVideo(bool vid) async {
    ...
    if (vid) {
      ...
      _participantNotifier!
          .updateVideoStatus(db, prefs!.getInt(Strings.userId), 1);
      updateVideoStatus(1);
    } else {
      await localParticipant?.muteVideo();

      _participantNotifier!
          .updateVideoStatus(db, prefs!.getInt(Strings.userId), 0);
      isLocalVideo = false;
      isFullVideo = false;
      updateVideoStatus(0);
    }
  ...
  void switchOffSocket() async {
    ...
    audioSocketService!
        .getSocket()!
        .off(ON_GET_PARTICIPANTS, onGetParticipants);
                                ...
                                itemBuilder: (BuildContext context,
                                  ...
                                  return (index <
                                    ...
                                    appSpeakerComponent(
                                      ...
                                      participantId:
                                      _participantNotifier!
                                          .getFloorsParticipants()![index].participantId,
                                      userId: prefs!.getInt(
                                          Strings.userId),
                                      imageUrl: Config
                                          .BASE_URL +
  ...
  void _connectNex() async {
    NexgeRTC.on('error', (eventName, data) {
      print(
          'STATUS------------------------------------------------------: code = ${data['code']} reason = ${data['message']}');
    ...
    NexgeRTC.on(ConferenceEvents.JOINED_CONFERENCE,
          ...
          print(
              'STATUS------------------------------------------------------: ${localParticipant?.participantId}');
          setState(() {
    ...
    NexgeRTC.on(ConferenceEvents.REMOTE_PARTICIPANT_JOINED,
          ...
          print(
              "Remote participant joined----------------------- - '${remoteParticipant?.participantId}'");
          RTCVideoRenderer _remoteRenderer = RTCVideoRenderer();
          await _remoteRenderer.initialize();
          _remoteRenderer.srcObject = remoteParticipant?.stream;

          String? id = remoteParticipant?.participantId;
          ...
          print(
              "Remote participant _remoteRenders length----------------------- - '${_remoteRenders.length}'");
          ...
          remoteParticipant?.on(ConferenceEvents.AUDIO_MUTE_CHANGED,
                ...
                print(
                    "Remote participant audio----------------------- - '${rp.isAudioMuted()}'");
              });

          remoteParticipant?.on(ConferenceEvents.VIDEO_MUTE_CHANGED,
                  (String eventName, Participant rp) {
                print(
                    "Remote participant video----------------------- - '${rp.isVideoMuted()}'");
              });
        });
    ...
    print('tokken success----------------------------------------------------------------------------------------------- = $authToken');

    print('channel name----------------------------------------------------------------------------------------------- '+channelName.toString());

    bool success = await NexgeRTC.join(authToken!, channelName!, participantId!, true, true);
...
/*    audioSocketService.getSocket().emit(EMIT_ON_REPORT, payload);*/
  ...
  void onRemove(dynamic data) {
    ...
    _participantNotifier!
        .deleteSingleItem(db, res.rows!.participantId, res.rows!.personType);
  ...
  void getUpdatedModerators(dynamic data) {
    ...
    _participantNotifier!
        .updateModerators(db, res.participantId, res.isModerator);
  }
  ...
  onError(dynamic data) {
    ...
    ToastBuilder().showSnackBar(
        res.message ?? "", sctx!, HexColor(AppColors.information));
    return data;
  }
  ...
  onVideoStatusChange(dynamic data) async {
    ...
    if (res != null && res.participantId != null && res.isVideoOn != null)
      _participantNotifier!
          .updateVideoStatus(db, res.participantId, res.isVideoOn!);
  }
  ...
  onGetParticipants(dynamic data) {
    ...
    if (res.rows!.listType != null) {
      Future(() {
        if (res.rows!.membersList != null &&
            res.rows!.membersList!.isNotEmpty) {
          ...
          _participantNotifier!
              .saveData(db, res.rows!.membersList!, res.rows!.listType);
        }
      });
    } else {
  ...
  onJoinSuccess(dynamic data) {
    ...
    if (res.statusCode == Strings.success_code) {
      ...
      authToken=res.rows!.nexToken ?? null;
      ...
      if (res != null) {
        log("JoinSuccess our socket ------------------------------------------------------------------------------------------------" +
            data.toString());
        var participant = ParticipantListItem.fromJson(res.rows!.toJson());

        _participantNotifier!.saveSingleItem(
            db,
            participant,
            res.rows!.role == EVENT_ROLE_TYPE.listener.type
                ? "floor"
                : "stage");
      }
    } else {
  ...
  onMicStatusChange(dynamic data) {
    ...
    if (res.participantId == int.parse(participantId!)) {
      ...
      // engine!.instance()!.muteLocalAudioStream(res.isSpeakerOn == 1 ? false : true);
  ...
  onLeave(data) {
    ...
    if (res != null)
      _participantNotifier!
          .deleteSingleItem(db, res.rows!.participantId, "floor");
  }
  ...
  handRaiseUpdate(data) async {
    ...
    if (res.isAccepted!) {
      ...
      await _participantNotifier!
          .updateModerators(db, item.participantId, item.isModerator);
  ...
  updateHandRaise(UpdateHandRaiseRequest requestPayload) {
    ...
    audioSocketService!
        .getSocket()!
        .emit(EMIT_HAND_RAISE_UPDATE, requestPayload);
    getRequestsCount();
  }
  ...
  Widget _fullVideoWidget(bool isLocal, int? id, RTCVideoRenderer? renderer) {
    return Align(
        ...
        children: [
          Container(
              child: isLocal
                  ? SizedBox.expand(
                  child:
                  Center(child: RTCVideoView(renderer!, mirror: true)))
                  : Center(child: RTCVideoView(renderer!, mirror: true))),
          Align(
