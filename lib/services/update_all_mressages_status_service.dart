...
class UpdateAllMessagesService {
  ...
  void updateAllMessages(DatabaseHelper db,
    ...
    if (listMessages != null && listMessages.length > 0) {
      for (var item in listMessages) {
        if (item.messageById!=userId &&( item.messageStatus != null && item.messageStatus == "sent" ||
            item.messageStatus == "delivered")) {
          MessagePayload messageDataData = getMessagePayload(item,userId,messageToId,messageWithType);
          print(jsonEncode(messageDataData)+
              "updating --------------------------------------------------------------chat page page");
